...
export default function HomePage() {
  ...
  // Se o Clerk não carregar em 8 segundos, mostra tela de erro de configuração
  useEffect(() => {
    if (!isLoading) return;
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center relative overflow-hidden">
      ...
      {loadingTimeout ? (
        // Clerk não inicializou — chave ausente ou inválida no ambiente de produção
        <div className="z-10 max-w-md w-full mx-4 bg-slate-900/80 border border-yellow-500/30 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl flex flex-col items-center gap-5 border-b-4 border-b-yellow-500">
          ...
          <div className="text-center">
            ...
            <p className="text-slate-400 text-xs font-semibold leading-relaxed">
              O sistema de login (Clerk) não conseguiu carregar. Isso normalmente acontece quando as variáveis de ambiente não estão configuradas corretamente na Vercel ou quando o site não foi reimplantado após configurá-las.
            </p>
          </div>
          ...
          <button
            ...
            className="w-full py-3 bg-yellow-500 hover:bg-yellow-400 text-slate-950 font-black rounded-2xl text-sm transition-all cursor-pointer"
          >
            Tentar Novamente
          </button>
        </div>
